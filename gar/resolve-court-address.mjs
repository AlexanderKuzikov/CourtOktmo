...
function normalize(addr) {
  ...
  // 1. Удалить точки после известных префиксов
  ...
  a = a.replace(/\b(зд|соор|влд)\.?\s/g, 'д ');

  // 4. Проспект
  a = a.replace(/\bпр\.(?=\s+[А-ЯЁ])/g, 'пр-кт');
  a = a.replace(/\bпроспект(?=\s)/g, 'пр-кт');
  a = a.replace(/\bпр-т\.?(?=\s)/g, 'пр-кт');

  // 5. кв → к
  a = a.replace(/\bкв\.?\s/g, 'к ');
